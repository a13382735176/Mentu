...
# dataload
data_path = '/home/user10/tao/dataset/preprocessed/BGL/200l_bgl.csv'
...
# Replace Conan-embedding model with RoBERTa
...
embeddings_path = '/home/user10/tao/dataset/preprocessed/BGL/200l_dim400_embeddings with no translated.npy'
...
def generate_pseudo_labels_kmeans(embeddings, n_clusters=2, sample_size=10000):
    ...
    # Calculate the distance of each sample to its cluster center
    distances = np.linalg.norm(embeddings - kmeans.cluster_centers_[kmeans.labels_], axis=1)
    
    # Define a distance threshold to remove difficult-to-classify samples (e.g., distance above the 90th percentile)
    threshold = np.percentile(distances, 70)  # 70 is used for BGL 100l
    valid_indices = sampled_indices[distances < threshold]
    
    return kmeans.labels_[distances < threshold], valid_indices

# Generate pseudo-labels from the training set, selecting 10,000 training samples
# Use KMeans to generate pseudo-labels
pseudo_labels_train, valid_train_indices = generate_pseudo_labels_kmeans(np.array(train_data['Embeddings'].tolist()), n_clusters=2)

# Replace real labels with pseudo-labels, only effective for valid_train_indices
train_data.loc[valid_train_indices, 'Status'] = pseudo_labels_train

# Save the valid embeddings and labels
...
# Assume the class with more labels is the normal sample, and the class with fewer is the anomalous sample
label_counts = np.bincount(pseudo_labels_train)
normal_label = np.argmax(label_counts)
anomalous_label = np.argmin(label_counts)

# Get the indices of normal and anomalous samples
...
# Store embeddings of normal and anomalous samples
storage_embeddings = embeddings[storage_indices]
storage_labels = pseudo_labels_train[storage_indices]  # Use pseudo-labels instead of real labels
...
# Calculate the probability of positive samples (normal samples) based on pseudo-labels
...
# Extract top-k probabilities and reconstruction error information corresponding to the last 50% indices
...
# Combine these data into input for training the attention mechanism
...
labels_test = test_data['Status'].values  # Use true labels
...
for epoch in range(num_attention_epochs):
    ...
    probabilities = torch.sigmoid(weighted_scores_train)
    loss = criterion(probabilities, labels_train)
    loss.backward()
...
# Disconnect the calculation graph and convert Tensor to a NumPy array
