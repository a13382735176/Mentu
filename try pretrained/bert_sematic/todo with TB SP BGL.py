...
data_path = '/home/user10/tao/dataset/preprocessed/BGL/20l_bgl.csv'
...
embeddings_path = '/home/user10/tao/dataset/preprocessed/BGL/20l_dim768_embeddings with no translated.npy'
...
max_normal_samples = min(5000, len(normal_indices))
...
weighted_scores_test = np.array(c)  # 转换为 NumPy 数组
# 假设 re_error_list 是一个包含数值的列表
#sorted_list = sorted(re_error_list, reverse=True)  # 对列表进行降序排序
#second_largest = sorted_list[1]  # 选择倒数第二大的值
#re_error_max = 0.0016
#print(re_error_max)
throld = np.percentile(weighted_scores_test, 95)  # 计算 95% 分位数的阈值
